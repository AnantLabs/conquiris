...
import net.conquiris.api.search.ManagedReaderSource;
...
/**
 * Default managed reader source implementation.
 * @author Andres Rodriguez
 */
abstract class DefaultManagedReaderSource implements ManagedReaderSource {
	/** Unmanaged reader source. */
	 ...
	 */
	DefaultManagedReaderSource(UnmanagedReaderSource unmanaged, long holdTime) {
	...
	public final Disposable<IndexReader> get() {
		while (true) {
			...
			final Disposable<IndexReader> ref = tryGet(opened);
			if (ref != null) {
				return ref;
			}
		}
	 ...
	 */
	private synchronized Disposable<IndexReader> tryGet(IndexReader opened) {
		boolean used = false;
		try {
			if (managed == null) {
				if (opened != null) {
